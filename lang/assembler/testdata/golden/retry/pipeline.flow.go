...
// RetryIngests carries one typed ingest per source statement in the retry flow.
//
...
type RetryIngests struct {
	// Ingest feeds the ingest source.
	Ingest machine.Ingest[Order]
}

// WireRetry declares the retry flow on m and returns its ingests.
//
// IT RETURNS RetryIngests AND error UNCONDITIONALLY. A flow with no
...
func WireRetry(m *machine.Machine) (RetryIngests, error) {
	ingest, ingest_ingest := m.Source[Order]("ingest", machine.WithEdge(Poll()))
	enrich := ingest.Map("enrich", Lookup)
	backoff := enrich.Map("backoff", Wait)
	done := enrich.Map("done", Store)
	backoff.Send(ingest)
