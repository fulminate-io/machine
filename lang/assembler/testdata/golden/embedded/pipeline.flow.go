...
//
// Flows in this file: screening, embedded.
package generated
...
// EmbeddedIngests carries one typed ingest per source statement in the embedded flow.
//
...
type EmbeddedIngests struct {
	// Ingest feeds the ingest source.
	Ingest machine.Ingest[Order]
}

// WireEmbedded declares the embedded flow on m and returns its ingests.
//
// IT RETURNS EmbeddedIngests AND error UNCONDITIONALLY. A flow with no
...
func WireEmbedded(m *machine.Machine) (EmbeddedIngests, error) {
	ingest, ingest_ingest := m.Source[Order]("ingest", machine.WithEdge(Poll()))
	count := ingest.Map("count", Count)
	screen_ok, screen_bad := count.If("screen.triage", Interesting)
	store := screen_ok.Map("store", Store)
	store.Drop("store#drain")
	screen_bad.Drop("bad#drop")

	return EmbeddedIngests{
